...
import net.nemanjakovacevic.recyclerviewswipetodelete.adapter.AdapterSwipedDelete;
...
public class Main4Activity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.menu_item_undo_checkbox) {
            ...
            ((AdapterSwipedDelete)mRecyclerView.getAdapter()).setUndoOn(item.isChecked());
        }
        if (item.getItemId() == R.id.menu_item_add_5_items) {
            //((AdapterSwipedDelete)mRecyclerView.getAdapter()).addItems(5);
        }
    ...
    private void setUpRecyclerView() {
        ...
        mRecyclerView.setAdapter(new AdapterSwipedDelete(new ArrayList<String>(), mRecyclerView));
        mRecyclerView.setHasFixedSize(true);

        AdapterSwipedDelete adapter = (AdapterSwipedDelete) mRecyclerView.getAdapter();
    }
}
